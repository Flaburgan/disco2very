...
import { hearts, nextCard, playedCards, startDefaultGame } from "./helpers/game";
...
test("starting a default game shows the board", async ({ page }) => {
  ...
  await expect(playedCards(page).locator('[class*="item-card_bottom__"]')).toContainText(/\d+(\.\d+)? kg CO/);
  // The next card is face down.
  await expect(nextCard(page)).toBeVisible();
  await expect(nextCard(page).locator('[class*="item-card_bottom__"]')).toContainText("? kg CO");
...
test("categories can be picked before starting a game", async ({ page }) => {
  ...
  await expect(page.getByText("Select the categories you want to play with:")).toBeVisible();
...
test("the back button returns from the categories to the instructions", async ({ page }) => {
  await page.goto("/");
  await page.getByRole("button", { name: "Pick categories" }).click();
  await expect(page.getByText("Select the categories you want to play with:")).toBeVisible();
  ...
  await expect(page.getByText("Select the categories you want to play with:")).toHaveCount(0);
});
